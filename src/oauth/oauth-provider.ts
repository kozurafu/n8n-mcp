...
export class OAuthProvider {
  ...
  verifyAccessToken(token: string): VerifiedAccessToken {
    ...
    if (!record || record.kind !== 'access' || record.revoked) {
      throw new OAuthErrorResponse('invalid_token', 'token is not valid', 401);
    }
    if (record.expires_at < Date.now()) {
      throw new OAuthErrorResponse('invalid_token', 'token expired', 401);
    }
    return {
      token,
